...
export default {
  ...
  data() {
    return {
      apiUrlLive: window.location.origin + '/evrinoma/api/live_video',
      apiUrlWowzaStream: window.location.origin + '/evrinoma/api/live_video/streaming_engine',
      apiUrlControl: window.location.origin + '/evrinoma/api/live_video/control',
      group: null,
      groupClass: 'Evrinoma\\LiveVideoBundle\\Dto\\LiveVideoDto',
      controlClass: 'Evrinoma\\LiveVideoBundle\\Dto\\LiveStreamDto',
      steamEngine: null,
      wowzaLink: "wowza_",
  ...
  methods: {
    ...
    _axiosResponse(type, response, query) {
      switch (type) {
        ...
        case 'wowza-error-stream':
          break;
        case 'control-action':
          setTimeout(this._resetLoad, (this.bufferSec + 5) * 1000, query.cam, query.query.action);
          break;
    ...
    doAction(cam, action) {
      ...
      let query = {class: this.controlClass, action: action, live_stream: cam.stream};
      axios
          .get(this.apiUrlControl, {params: query})
          .then(response => (this._axiosResponse('control-action', response, {query: query, cam: cam})))
